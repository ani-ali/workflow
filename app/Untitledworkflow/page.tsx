...
export default function NodeEditor() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#000000] text-zinc-300 font-sans overflow-hidden">
      ...
      {/* WRAPPER FOR LEFT UI - This entire block hides when isSidebarOpen is false */}
      {isSidebarOpen && (
        <div className="flex animate-in slide-in-from-left duration-300 ease-out h-full">
          ...
          <aside className="w-[72px] flex flex-col items-center py-4 border-r border-white/5 bg-[#080808] z-30">
            ...
            <nav className="flex flex-col gap-2 items-center flex-1">
              ...
              <IconButton icon={<Plus size={20} />} label="Nodes" active={true} />
              <IconButton icon={<Search size={20} />} label="Search" />
            </nav>
          </aside>

          {/* 2. EXPANDABLE NODES SIDEBAR */}
          <aside className="w-[300px] bg-[#0c0c0c] border-r border-white/5 flex flex-col z-20">
            <div className="h-[55px] flex items-center justify-between px-5 border-b border-white/5">
              {isEditing ? (
                <input
                  ...
                  className="bg-transparent text-[18px] font-bold text-white outline-none w-full border border-white/10 rounded-xl px-2"
                />
              ) : (
                <h1 
                  onClick={() => setIsEditing(true)}
                  className="text-[18px] font-bold text-white tracking-tight cursor-pointer hover:opacity-80 transition-opacity truncate"
                >
                  {projectName}
                </h1>
              )}
              
              {/* Internal Toggle - Hides both */}
              <div 
                className="p-1.5 hover:bg-white/5 rounded-lg cursor-pointer transition-colors"
                onClick={() => setIsSidebarOpen(false)}
              >
                <PanelRight size={18} className="text-zinc-500 rotate-180" />
              </div>
            </div>
            ...
            <div className="p-4 mt-3 pt-0 flex-1 overflow-y-auto no-scrollbar">
              ...
              <div className="relative mb-6">
                ...
                <input type="text" placeholder="Search nodes or models" className="w-full bg-white/[0.02] border border-white/5 rounded-xl py-2 pl-10 pr-4 text-[13px] focus:outline-none focus:border-white/20 transition-all placeholder:text-zinc-700" />
              </div>
      ...
      <main className="flex-1 relative bg-[#050505]">
        ...
        {/* FLOATING TOOLBAR */}
        <div className="absolute top-6 left-8 flex items-center bg-[#111111] border border-white/10 pl-2 pr-3 py-1.5 rounded-full shadow-2xl transition-all hover:border-white/20 z-30">
                        <Image src="/floww.png" alt="Logo" width={25} height={25} />

          <div className="w-px h-5 bg-white/10 mx-3" />
          <span 
            onClick={() => setIsEditing(true)}
            className="text-[14px] font-medium text-white/90 tracking-tight cursor-pointer hover:text-white mr-4"
          >
            {projectName}
          </span>
          
          {/* Main Toggle Button - Hides/Shows the entire Sidebar + Nav strip */}
          <div 
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className={`flex items-center justify-center w-7 h-7 rounded-lg  cursor-pointer transition-all ${isSidebarOpen ? '' : ''}`}
          >
            
            <PanelRight size={14} className={isSidebarOpen ? "" : "rotate-180"} />
          </div>
        </div>
      </main>
    </div>
