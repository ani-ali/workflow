  ...
  const handleGenerateText = async () => {
    ...
    try {
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ prompt:"generate medium size response don't write additional stuff only what I asked for PROMPT IS :" + promptText }),
      })
    ...
    <div className="custom-node">
      <div className="bg-[#171717] rounded-4xl min-w-[300px] max-w-[300px] overflow-hidden relative">
       <div className='m-3 rounded-2xl min-h-96 max-h-auto bg-[#212121]'>
        {isLoading ? <p className='text-xs text-gray-400 font-bold px-2 py-4 animation-palse'>Loading...</p> : (generatedText ? (
       ...
       <div className="m-2 w-[260px] mb-5 hover:border-[0.5px] hover:border-gray-500 border border-gray-500/90 bottom-0 absolute p-1.5 rounded-md bg-[#212121] ">
          <textarea
            value={displayText}
            onChange={(e) => {setText(e.target.value);setTextChange(true)}}
            placeholder="Enter your prompt here or connect input nodes..."
            className="w-full bg-transparent text-white placeholder-gray-500 text-sm resize-none outline-none min-h-[120px] pr-12"
      ...
      {/* Output handle to pass generated text to other nodes */}
      <Handle
        type="source"
